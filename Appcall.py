...
if user_input:
    ...
    st.markdown(
        ...
        <div style='background-color: {rbc_canada_palette["secondary"]}; padding: 10px;'>
            <p style='color: {rbc_canada_palette["primary"]}; font-size: 12px;'>
                {model_output}
            </p>
        </div>
        """,
...
st.sidebar.write(
    "The App responds to your questions on HSBC RBC Transition based on the RBC Product document located at link"
)
RBC_doc_url = "https://www.rbc.com/hsbc-canada/product-service-guide.html?#personal"
st.sidebar.markdown(
    f'<a href="{RBC_doc_url}">Link to RBC Document</a>', unsafe_allow_html=True)
